...
for dir in "$dir_augerat" "$dir_christofides" "$dir_uchoa"; do
    for algorithm in "${algorithms[@]}"; do
        ...
        for file in "$dir"/*.xml; do
            ...
            /usr/bin/python3.10 ../data_analysis/main.py "$file" "$output_file" --solution_file "$solution_file"
            echo /usr/bin/python3.10 ../data_analysis/main.py "$file" "$output_file" --solution_file "$solution_file"
